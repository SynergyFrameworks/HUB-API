...
#pragma warning disable 1066,1570,1573,1591
    ...
    /// </summary>
    public class Database : IDisposable, IDatabase, IStatementDBExtensions
    {
        #region IDisposable
        ...
        protected virtual bool IsNew(string primaryKeyName, PocoData pd, object poco)
        {
            ...
            if (type.GetTypeInfo().IsGenericType && type.GetGenericTypeDefinition() == typeof(Nullable<>) || !type.GetTypeInfo().IsValueType)
                return pk == null;
    ...
    public class ColumnInfo
    {
        ...
        public static ColumnInfo FromProperty(PropertyInfo propertyInfo)
        {
            ...
            var colAttrs = propertyInfo.GetCustomAttributes(typeof(ColumnAttribute), true).ToArray();
            if (explicitColumns)
            {
            ...
            {
                if (propertyInfo.GetCustomAttributes(typeof(IgnoreAttribute), true).ToArray().Length != 0)
                    return null;
            }
    ...
    public abstract class DatabaseProvider : IProvider
    {
        ...
        internal static IProvider Resolve(Type type, bool allowDefault, string connectionString)
        {
            ...
            //if (typeName.IndexOf("OleDb", StringComparison.OrdinalIgnoreCase) >= 0
            //    && (connectionString.IndexOf("Jet.OLEDB", StringComparison.OrdinalIgnoreCase) > 0 || connectionString.IndexOf("ACE.OLEDB", StringComparison.OrdinalIgnoreCase) > 0))
            //{
        ...
        internal static IProvider Resolve(string providerName, bool allowDefault, string connectionString)
        {
            ...
            if (providerName.IndexOf("MySql", StringComparison.OrdinalIgnoreCase) >= 0)
                return Singleton<MySqlDatabaseProvider>.Instance;
            if (providerName.IndexOf("MariaDb", StringComparison.OrdinalIgnoreCase) >= 0)
                return Singleton<MariaDbDatabaseProvider>.Instance;
            if (providerName.IndexOf("SqlServerCe", StringComparison.OrdinalIgnoreCase) >= 0 ||
                providerName.IndexOf("SqlCeConnection", StringComparison.OrdinalIgnoreCase) >= 0)
                return Singleton<SqlServerCEDatabaseProviders>.Instance;
            ...
            if (providerName.IndexOf("Firebird", StringComparison.OrdinalIgnoreCase) >= 0 ||
                providerName.IndexOf("FbConnection", StringComparison.OrdinalIgnoreCase) >= 0)
                return Singleton<FirebirdDbDatabaseProvider>.Instance;
            ...
            if (providerName.IndexOf("SqlServer", StringComparison.OrdinalIgnoreCase) >= 0 ||
                providerName.IndexOf("System.Data.SqlClient", StringComparison.OrdinalIgnoreCase) >= 0)
                return Singleton<SqlServerDatabaseProvider>.Instance;
        ...
        public static IMapper GetMapper(Type entityType, IMapper defaultMapper)
        {
            ...
            try
            {
                ...
                if (_mappers.TryGetValue(entityType, out val))
                    return val;
                if (_mappers.TryGetValue(entityType.GetTypeInfo().Assembly, out val))
                    return val;
    ...
    public class PocoColumn
    {
        ...
        public virtual object ChangeType(object val)
        {
            ...
            if (val.GetType().GetTypeInfo().IsValueType && PropertyInfo.PropertyType.GetTypeInfo().IsGenericType && PropertyInfo.PropertyType.GetGenericTypeDefinition() == typeof(Nullable<>))
                t = t.GetGenericArguments()[0];
    ...
    public class PocoData
    {
            ...
            return PocoFactories.Get(key, () =>
            {
                ...
                else
                {
                    ...
                    var fnOnLoaded = RecurseInheritedTypes<MethodInfo>(Type,
                        (x) => x.GetMethod("OnLoaded", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic));
                    if (fnOnLoaded != null)
                    {
                        il.Emit(OpCodes.Dup);
        ...
        private static Func<object, object> GetConverter(IMapper mapper, PocoColumn pc, Type srcType, Type dstType)
        {
            ...
            {
                if (dstType.GetTypeInfo().IsEnum && srcType == typeof(string))
                {
        ...
        public static TableInfo FromPoco(Type t)
        {
            ...
            if (string.IsNullOrEmpty(ti.PrimaryKey))
            {
                ...
                if (prop != null)
                {
                    ...
                    ti.AutoIncrement = prop.PropertyType.GetTypeInfo().IsValueType;
                }
            }
    ...
    internal static class EnumMapper
    {
        ...
        public static object EnumFromString(Type enumType, string value)
        {
            Dictionary<string, object> map = _types.Get(enumType, () =>
            {
                ...
                var newmap = new Dictionary<string, object>(values.Length, StringComparer.OrdinalIgnoreCase);
